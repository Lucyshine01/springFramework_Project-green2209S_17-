...
if(randomSearch == 1) $("#search2").val("인테리어");
else if(randomSearch == 2) $("#search2").val("상업공간");
else if(randomSearch == 3) $("#search2").val("조명");
else if(randomSearch == 4) $("#search2").val("욕실");
else if(randomSearch == 5) $("#search2").val("타일");
else if(randomSearch == 6) $("#search2").val("페인트");
else if(randomSearch == 7) $("#search2").val("싱크대");
else if(randomSearch == 8) $("#search2").val("도배장판");
else if(randomSearch == 9) $("#search2").val("필름");
else if(randomSearch == 10) $("#search2").val("도면");
else if(randomSearch == 11) $("#search2").val("모델링");
else if(randomSearch == 12) $("#search2").val("컨설팅");
*/
...
function infoModalOn(info) {
	$("#info").html(info+'');
	$("#infoModal").show();
	$("#infoModal").animate({opacity:"1"},200);
}
function reportModalOn(idx) {
	$("#reportBtn").attr("onclick","replyReport("+idx+")");
	$("#reportModal").show();
	$("#reportModal").animate({opacity:"1"},200);
	let pageHeight = document.body.scrollHeight+100;
	$(".modalBack").css("height", pageHeight + "px");
}
function modalClose() {
	$(".modalCss").hide();
...
$(document).ready(function(){
  ...
  $("#loading_Bar").animate({width: deviceWidth + "px"},500);
  // $("#loading_Bar").slideUp();
  ...
  $(".help").css("transition","0.3s");
  $("#search1").css("transition","0.3s");
